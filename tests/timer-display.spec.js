...
test.describe('Timer Calculations and Display', () => {
  ...
  test('should calculate correct duration for exams', async ({ page }) => {
    ...
    for (const tc of testCases) {
      ...
      const row = page.locator('tbody tr', { hasText: `Duration ${tc.expected}` });
  ...
  test('should handle different extra time values', async ({ page }) => {
    const extraTimes = [
      { extra: 15, expectedFinish: '11:15', expectedExtra: 'Extra Time 15 mins' },
      { extra: 25, expectedFinish: '11:25', expectedExtra: 'Extra Time 25 mins' },
      { extra: 45, expectedFinish: '11:45', expectedExtra: 'Extra Time 45 mins' },
    ];
  ...
  test('should handle timer reset when start button clicked', async ({ page }) => {
    // Add an exam
    await addExam(page, {
      ref: 'TEST',
      subject: 'Test Exam',
  ...
  test('should calculate duration across different hour ranges', async ({ page }) => {
    const testCases = [
  ...
  test('should calculate finish time correctly with various durations', async ({ page }) => {
    const testCases = [
    ...
    for (const tc of testCases) {
      ...
      const row = page.locator('tbody tr', { hasText: `Finish ${tc.expectedFinish}` });
